var processStripePayment = (data, base_url, public_key, modal) => {
    $("head").jsCssLoader(
        'https://js.stripe.com/v3/', 'js', function(e) {
            ...
            if ($(modal+".modal").find(".payment-processor").lenght>=1) {
                $(".payment-processor").remove();
            }
            $(modal+".modal.fade.show .modal-dialog").removeClass("modal-lg").removeClass("modal-sm").removeClass("modal-md");
            $(modal+".modal.fade.show .modal-dialog .modal-content").html(form);
            loading(true); 
            ...
            $.post(link('ajax/payments/stripe/payment_intents'), purchase, function(resp) {
                if (resp.success === true) {
                    ...
                    cardElement.on('ready', function(e) {
                        loading(false);
                    });
                    ...
                    $("#payment-form .payment-notification").alert_notice(resp.message, "error", false);
                }
            })
        }, 
